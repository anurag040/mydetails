...
@Component({
  ...
  imports: [
    ...
    MatStepperModule, MatDividerModule, MatFormFieldModule, MatProgressSpinnerModule,
...
export class ProjectGeneratorComponent {
  ...
  // Derived selection - simplified to avoid circular dependencies
  ...
  generate() {
    ...
    this.projectService.uploadPRD(prdFile, 'Generated Project').subscribe({
      ...
      error: (error) => {
        this.isUploading.set(false);
        this.snack.open('PRD upload failed. Please try again.', 'OK', { duration: 5000 });
