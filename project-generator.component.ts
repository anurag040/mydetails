...
type DbOption = 'Vertica' | 'Snowflake' | 'Oracle' | 'PostgreSQL' | 'MySQL' | 'MongoDB';
type AuthOption = 'JWT' | 'OAuth2' | 'Basic' | 'LDAP';
type DeploymentOption = 'Docker' | 'Kubernetes' | 'AWS' | 'Azure' | 'GCP';
...
export class ProjectGeneratorComponent {
  ...
  selectedDatabasesCount = computed(() => (this.f.databases.value || []).length);
  selectedAuthCount = computed(() => (this.f.authentication.value || []).length);
  selectedDeploymentCount = computed(() => (this.f.deployment.value || []).length);
  ...
  togglePanel(panel: 'basic' | 'advanced' | 'optional') {
    const current = this.panelOpenState();
